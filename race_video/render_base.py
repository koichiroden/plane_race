...
from .geo import (compute_projection, compute_fixed_projection, project, unproject,
                   CANVAS_W, CANVAS_H, MAP_BOTTOM, SAFE_TOP_Y)
...
def render_base_map(config, paths, geojson_path="data/routes.geojson"):
    ...
    map_extent = config.get("map_extent", "route")  # "route"(既定・従来通り) | "japan"(固定・全国表示)
    if map_extent == "japan":
    ...
    coastline_name = "coastline_japan.geojson" if map_extent == "japan" else "coastline.geojson"
